...
int main(int argc, char** argv) {
    ...
    // Для 1 процесса используем упрощенную версию
    if (size == 1) {
        ...
        // Простое умножение для 1 процесса
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                double sum = 0.0;
                for (int k = 0; k < N; k++) {
        ...
        printf("Cannon Algorithm (1 process): %dx%d\n", N, N);
    ...
    // Создаем подтип для блока матрицы
    MPI_Datatype block_type;
    MPI_Type_vector(block_size, block_size, N, MPI_DOUBLE, &block_type);
    MPI_Type_commit(&block_type);
    
    // Распределяем блоки A и B
    if (rank == 0) {
        // Главный процесс копирует свой блок
        for (int x = 0; x < block_size; x++) {
            for (int y = 0; y < block_size; y++) {
                local_A[x * block_size + y] = A[x * N + y];
                local_B[x * block_size + y] = B[x * N + y];
            }
        }
        
        // Рассылаем остальные блоки
        for (int i = 0; i < q; i++) {
            for (int j = 0; j < q; j++) {
                if (i == 0 && j == 0) continue;
                ...
                // Отправляем блок A
                double *a_start = &A[i * block_size * N + j * block_size];
                MPI_Send(a_start, 1, block_type, dest_rank, 0, MPI_COMM_WORLD);
                
                // Отправляем блок B
                double *b_start = &B[i * block_size * N + j * block_size];
                MPI_Send(b_start, 1, block_type, dest_rank, 1, MPI_COMM_WORLD);
            }
        }
    } else {
        // Остальные процессы получают свои блоки
        MPI_Recv(local_A, block_elements, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(local_B, block_elements, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    for (int step = 0; step < q; step++) {
        // Локальное умножение матриц (оптимизированный порядок циклов)
    ...
    // Эффективный сбор результатов
    MPI_Datatype block_resized_type;
    MPI_Type_create_resized(block_type, 0, block_size * sizeof(double), &block_resized_type);
    MPI_Type_commit(&block_resized_type);
    
    if (rank == 0) {
        // Главный процесс собирает результаты
        // Сначала копируем локальный блок
        for (int x = 0; x < block_size; x++) {
            for (int y = 0; y < block_size; y++) {
        ...
        for (int i = 0; i < q; i++) {
            for (int j = 0; j < q; j++) {
                ...
                MPI_Recv(temp_block, block_elements, MPI_DOUBLE, src_rank, 2, 
                        MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                
                for (int x = 0; x < block_size; x++) {
                    for (int y = 0; y < block_size; y++) {
                        C[(i * block_size + x) * N + (j * block_size + y)] = 
                            temp_block[x * block_size + y];
                    }
                }
                free(temp_block);
            }
